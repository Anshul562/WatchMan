        ...
        <fieldset>
          ...
          <div className="in-cast">
            {personCast.map((person) => (
              <Link to={'/content/' + ((person?.media_type === "movie") ? "m" : "t") + person?.id} key={person.id}>
                <motion.div animate={{ x: 0, opacity: 1 }} initial={{ x: 150, opacity: 0 }} transition={{ delay: 0.5 }} className="cast_columnParent">
                  <img onError={event => {
                                            ...
                                            event.onerror = null
                                        }} src={base_url + "/w220_and_h330_face" + person.poster_path} className="cast_columnChild" />
        ...
        <fieldset style={{ paddingTop: '20px' }}>
          ...
          <div className="in-cast">
            {personCrew.map((person) => (
              <>
                <Link to={'/content/' + ((person?.media_type === "movie") ? "m" : "t") + person?.id}>
                  <motion.div animate={{ x: 0, opacity: 1 }} initial={{ x: 150, opacity: 0 }} transition={{ delay: 0.75 }} className="cast_columnParent">

                    <img  src={base_url + "/w220_and_h330_face" + person.poster_path} className="cast_columnChild" key={person.id} onError={event => {
                                            event.target.src = altimg
                                            event.onerror = null
                                        }}/>
